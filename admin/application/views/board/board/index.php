    ...
    <div class="row">
        <div class="col-sm-2">
            <div class="form-group">
                ...
                <select v-model="schFields.type" class="form-control">
                    <option value="">전체</option>
                    <option v-for="item in sub_type" :value="item.code">{{ item.name }}</option>
                </select>
            </div>
        </div>
        <div class="col-sm-1">
            <div class="form-group">
                ...
                <input type="text" v-model="schFields.title" v-on:keyup.enter="readDataFromAPI('s')" class="form-control">
            </div>
        </div>
        <div class="col-sm-2">
            <div class="form-group">
        ...
        <div class="col-sm-1 text-right">
            <div class="form-group">
                ...
                <div class="input-group-btn"><button class="btn btn-primary btn-xs" @click="readDataFromAPI('s')">검색 </button></div>
            </div>
        </div>
    </div>
</div>
                        ...
                        >
                            <template v-slot:top>
                                <v-toolbar flat >
                                            ...
                                            <v-card-text>
                                                ...
                                                <v-container>
                                                    ...
                                                    <v-row>
                                                        ...
                                                        <v-col cols="8" sm="6" md="4">
                                                            <v-text-field persistent-hint v-model="editedItem.source" label="출처"></v-text-field>
                                                        </v-col>
                                                    ...
                                                    <v-row>
                                                        <v-textarea v-model="editedItem.etc_data" outlined dense label="추가정보" value=""></v-textarea>
                                                        <div>
                                                            <span class="headline">* 줄로 데이터 구분을 하고 데이터는 key:value|key:value 형태로 입력<br/>
                                                            * 입력 예 => size:200|price:1000</span>
                                                        </div>
                                                    </v-row>
                                                    <v-row>
                                    ...
                                    <v-dialog v-model="dialogQuestion" max-width="800px" >
                                        <v-card>
                                            <v-card-title>
                                                <span class="headline">문항설정</span>
                                            </v-card-title>
                                            <v-divider></v-divider>
                                            <v-card-text>
                                                <v-form
                                                ...
                                                >
                                                    <v-container>
                                                        <v-row>
                                                            <v-col cols="8" sm="6" md="4">
                                                                <v-select
                                                                        v-model="questionItem.answer_type"
                                                                        :items="answerType"
                                                                        item-text="name"
                                                                        item-value="code"
                                                                        label="응답형태"
                                                            ...
                                                            <v-col cols="8" sm="6" md="4">
                                                                <v-select
                                                                        v-model="questionItem.answer_type_fix"
                                                                        :items="answerFixType"
                                                                        item-text="name"
                                                                        item-value="code"
                                                                        label="응답텍스트 문항별 노출"
                                                        ...
                                                        <v-row>
                                                            <v-textarea v-model="questionItem.question_text" outlined dense label="문항정보" value=""></v-textarea>
                                                        </v-row>
                                                        <v-row>
                                                            <v-textarea v-model="questionItem.answer_data" outlined dense label="응답정보" value=""></v-textarea>
                                                        </v-row>
                                                        <v-row>
                                                            <v-textarea v-model="questionItem.result_text" outlined dense label="결과정보" value=""></v-textarea>
                                                        </v-row>
                                                        <v-row>
                                                            <v-col sm="20" style="color:#0D47A1">
                                                            <span class="headline">
                                                                * 문항정보 입력 형식 => 번호를 넣어 줄단위로 입력함, 분류|문항정보|기본응답정보예외점수 형태로 입력<br/>
                                                                * 응답정보 입력 예 => 줄단위로 입력, 점수|항목 형태로 입력<br/>
                                                                * 결과정보 입력 형식 => 분류||결과항목:점수~점수|결과항목:점수~점수||총문항수|충문항수기준으로점수산정Y,N</span>
                                                            </v-col>
                                                        </v-row>
                                                    </v-container>
                                                </v-form>
                            ...
                            <template v-slot:item.question="{ item }">
                                <v-btn class="test" small @click="questionSet(item,'main')">설정</v-btn>
                            </template>
                            <template v-slot:item.sub_question="{ item }">
                                <v-btn class="test" small @click="questionSet(item,'sub')">설정</v-btn>
                            </template>
        ...
        new Vue({
            ...
            data () {
                return {
                    ...
                    headers: [
                        { text: '제목', value: 'title' },
                        { text: '등록일', value: 'insert_dt'<?=$type != 'selftest'? "" : ", align: ' d-none'"?> },
                        { text: '노출', value: 'is_view' },
                        { text: '이미지', value: 'board_image' },
                        { text: '분류', value: 'sub_type_text'},
                        { text: '문항설정', value: 'question'<?=$type == 'selftest'? "" : ", align: ' d-none'"?> },
                        { text: '추가 문항설정', value: 'sub_question'<?=$type == 'selftest'? "" : ", align: ' d-none'"?> },
                        { text: '상세', value: 'data-table-expand'<?=$type != 'selftest'? "" : ", align: ' d-none'"?> },
                    ...
                    answerType: [{code : 'radiobutton', name : '라디오버튼'},{code : 'checkbox', name : '체크박스'}],
                    answerFixType: [{code : 1, name : '예'},{code : 0, name : '아니오'}],
                    editedItem: {
                        type:'<?=$type?>',
                        image_temp_no : 0,
                        sub_type : 0,
            ...
            methods: {
                ...
                editItem (item) {
                    ...
                    var etc_data = '';
                    //this.editedItem.etc_data = this.editedItem.etc_data.replaceAll('","',"\r\n").replace("[{","").replace("}]","").replaceAll('"','')
                   // this.questionItem.question_text = '';

                    for (let item of this.editedItem.etc_data) {
                        etc_data = (etc_data === '' ? '': "\r\n") + JSON.stringify(item).replaceAll('","',"|").replace("{","").replace("}","").replaceAll('"','')
                    }

                    this.editedItem.etc_data = etc_data;
                },
                ...
                close () {
                    ...
                    this.$nextTick(() => {
                        this.editedItem = Object.assign({}, this.defaultItem.board)
                        this.editedIndex = -1
                    })
                },
                getCode(sub_code){
                ...
                save () {
                    ...
                    axios
                    .post("/board/board/"+(this.editedIndex > -1 ? 'modify' : 'add'),formData)
                        .then((response) => {
                            if(response.data.success){
                                // this.showMessage.show = true;
                                // this.showMessage.message = response.data.message;
                                axios
                                    .get('/board/board/detail/'+response.data.result+'/simple')
                                    .then((res) => {
                                        if(this.editedIndex > -1){
                ...
                questionSave () {
                    ...
                    let formData = new FormData();
                    for(let key in this.questionItem) {
                        formData.append(key, this.questionItem[key]);
                    }

                    axios
                        .post("/etc/board/question_save",formData)
                        .then((response) => {
                            if(response.data.success){
                ...
                questionSet(item, mode){
                    this.questionItem = Object.assign({}, this.defaultItem.selftest)
                    if(mode == 'sub'){
                        this.questionItem.sort = 2
                    }

                    axios
                        .get("/etc/board/selftest_detail/"+item.board_no+'/'+this.questionItem.sort)
                        .then((res) => {
                            console.log(res)

                            this.questionItem.answer_type_fix = res.data.result.main.answer_type_fix;
                            this.questionItem.selftest_main_no = res.data.result.main.selftest_main_no;
                            this.questionItem.answer_type = res.data.result.main.answer_type;
                            this.questionItem.answer_data = '';
                            for (let item of res.data.result.main.answer_data) {
                                this.questionItem.answer_data += (this.questionItem.answer_data === '' ? '': "\r\n") + (item[0]+'|'+item[1]);
                            }

                            this.questionItem.question_text = '';
                            for (let item of res.data.result.question) {
                                this.questionItem.question_text += (this.questionItem.question_text === '' ? '': "\r\n") + (item.section+'|'+item.question+'|'+item.answer_type);
                            }

                            this.questionItem.result_text = '';
                            for (let item of res.data.result.result) {
                                let _text = '';
                                var sub_item = item['result_range'];
                                for (let sub_item of item['result_range']) {
                                    _text += (_text === '' ? '': "|") + (sub_item.title+':'+sub_item.range[0]+'~'+sub_item.range[1]);
                                }

                                this.questionItem.result_text += (this.questionItem.result_text === '' ? '': "\r\n") + (item.section+'||'+_text);
                            }
                        });

                    this.questionItem.board_no = item.board_no;
            ...
            mounted() {
                ...
                this.getCode('BOARD_TYPE');

                if(this.type == 'faq'){
                    this.getCode('FAQ_TYPE');
                }

                if(this.type == 'etc'){
                    this.getCode('BOARD_ETC_tYPE');
                }

                if(this.type == 'data'){
                    this.getCode('BOARD_DATA_SUB_TYPE');
                }

                if(this.type == 'selftest'){
                    this.getCode('SELFTEST_TYPE');
                }

                if(this.type == 'provision'){
                    this.getCode('PROVISION');
                }

                if(this.type == 'lecture'){
                    this.getCode('LECTURE_TYPE');
                }

                if(this.type == 'education'){
                    this.getCode('BOARD_EDUCATION_TYPE');
                }
            },
        });
