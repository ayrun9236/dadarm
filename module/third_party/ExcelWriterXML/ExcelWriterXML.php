 ...
 */
include(dirname(__FILE__).'/ExcelWriterXML_Sheet.php');
/**
 * Includes the other class file to create Styles
 */
include(dirname(__FILE__).'/ExcelWriterXML_Style.php');
...
class ExcelWriterXML{
     ...
     */
	function __construct($fileName = 'file.xml'){
		// Add default style
...
@param string $function The name of the function that was called
	...
	public function sendHeaders($filename = ''){
        $this->docFileName = ($filename)?$filename:$this->docFileName;
		$extt = explode('.',$this->docFileName);
		$ext = array_pop($extt);
