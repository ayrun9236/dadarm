...
abstract class REST_Controller extends CI_Controller {
    ...
    // 공용 코드
    const HTTP_NOT_OK = 600;                    // 일반 오류 코드
    const HTTP_DATA_EXISTS = 601;               // DATA EXISTS
    const HTTP_PAGE_REFRESH = 602;               // 페이지 새로고침


    // 회원 관련
    const HTTP_USER_LOGIN_FAIL = 700;           // 유저 로그인 실패
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->load->library('common/format');
        ...
        //log_message('error', print_r($this->request->method, true));
    ...
    public function _remap($object_called, $arguments = [])
    {
		...
		/* */
		//log_message('error', print_r($this->_head_args, true));
		if ($this->config->item('rest_ip_whitelist_enabled') === TRUE)
		{
			if(!isset($this->_head_args['Platform']) && $controller_method != 'board_update_count_get'){
				$this->_check_whitelist_auth();
			}
		}
		...
		if (method_exists($this, 'auth_method_check')) {
			$auth_array = $this->auth_method_check();

			if (method_exists($this, 'member_type')) {
                $member_type = $this->member_type();
            }
			if ( in_array($controller_method, $auth_array) === false ){
				$this->auth_user = $this->auth->check();
			}
			else {
				$this->auth_user = $this->auth->info();
			}
		}
	...
	public function response_error($message, $http_code)
	{
		$this->response(
			array(
				'success' => false,
	...
	public function parameter_validate($validate_array, $data=null)
	{
		$this->load->library('form_validation');
		$this->form_validation->set_error_delimiters('', '');

		$check_data = array();
		if ( count($this->head()) > 0 ) $check_data = array_merge($check_data, $this->head());
		if ( count($this->get()) > 0 ) $check_data = array_merge($check_data, $this->get());
		if ( count($this->post()) > 0 ) $check_data = array_merge($check_data, $this->post());
		if ( count($this->put()) > 0 ) $check_data = array_merge($check_data, $this->put());
		if ( count($this->delete()) > 0 ) $check_data = array_merge($check_data, $this->delete());
		if ( $data ) $check_data = array_merge($check_data, $data);
		$this->form_validation->set_data($check_data);
		$this->form_validation->set_rules($validate_array);
		if ($this->form_validation->run() === FALSE)
		{
			$this->response_error($this->form_validation->error_string(),  self::HTTP_BAD_REQUEST);
	...
	function get_codes($searchs = array()){
		...
		if(isset($searchs['parent_code'])){
			$ret = $this->code_model->sub_codes($searchs['parent_code'], isset($searchs['code'])  ? $searchs['code'] : '');
		}
